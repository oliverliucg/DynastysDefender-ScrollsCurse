...
// The Shooter class is a GameObject that is controlled by the player. It is the object that shoots the bubbles.
// It carries a bubble that it will shoot when the player presses the space bar.
// it also carries another that it's going to shoot next.
class Shooter : public GameObject {
public:
	// Constructs a new Shooter instance. Requires a position vector, size vector, rotation pivot vector, and a sprite.
	// Will Initialize the carriedBubble object at it rotation center and nextBubble object at its tail.
	Shooter(glm::vec2 pos, glm::vec2 size, glm::vec2 rotationPivot, glm::vec2 tailPivot, Texture2D sprite);
	~Shooter();
	...
	glm::vec4 GetNewBubbleColor(const std::unordered_map<int, std::unique_ptr<Bubble>>& statics);
