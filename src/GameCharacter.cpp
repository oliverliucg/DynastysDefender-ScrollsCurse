...
GameCharacter::GameCharacter(
    const std::string& name, GameCharacterState state,
    std::unordered_map<GameCharacterState, glm::vec2> positions,
    std::unordered_map<GameCharacterState, glm::vec2> sizes, float rotation,
    glm::vec2 pivot, std::unordered_map<GameCharacterState, Texture2D> textures)
    : GameObject(positions[state], sizes[state], rotation, pivot,
                 glm::vec2(0.0f, 0.0f), glm::vec4(1.0f, 1.0f, 1.0f, 1.0f),
                 textures[state]),
      name(name),
      state(state),
      positions(positions),
...
GameCharacter::GameCharacter(const GameCharacter& other) : GameObject(other) {
  name = other.name;
  state = other.state;
  health = other.health == nullptr ? nullptr
                                   : std::make_unique<Health>(*other.health);
...
GameCharacter::GameCharacter(GameCharacter&& other) noexcept
    : GameObject(other) {
  name = other.name;
  state = other.state;
...
void GameCharacter::DrawGameCharacter(
    std::shared_ptr<SpriteRenderer> sprideRenderer,
...
std::string GameCharacter::GetName() const { return name; }

GameCharacterState GameCharacter::GetState() const { return state; }
...
void GameCharacter::SetTargetPosition(const std::string& targetName,
                                      glm::vec2 position) {
  this->targetPositions[targetName] = position;
}

glm::vec2 GameCharacter::GetTargetPosition(
    const std::string& targetName) const {
  assert(this->targetPositions.count(targetName) > 0 &&
         "The target position is not set");
...
float GameCharacter::GetTargetRoll() const { return targetRoll; }

std::unordered_map<int, std::unordered_map<GameCharacterState, glm::vec2> >&
GameCharacter::GetRelativeCenterRatios() {
...
GameCharacter::GetCarriedObjects() {
...
void GameCharacter::AddCarriedObject(std::shared_ptr<GameObject> object) {
  ...
  assert(carriedObjects.find(object->GetID()) == carriedObjects.end() &&
         "The object to be carried is already carried by the character");
  carriedObjects[object->GetID()] = object;

  // Set the rotation pivot of the carried object to the rotation pivot of the
  // character
  glm::vec2 rotationPivotCoord = GetRotationPivotCoord();
  glm::vec2 rotationPivotForCarriedObject =
      (rotationPivotCoord - object->GetPosition()) / object->GetSize();
...
void GameCharacter::ActivateStun() { isStunned = true; }

void GameCharacter::DeactivateStun() { isStunned = false; }

bool GameCharacter::IsMoving() const { return isMoving; }

bool GameCharacter::IsRotating() const { return isRotating; }

bool GameCharacter::IsStunned() const { return isStunned; }
...
void GameCharacter::SetPosition(glm::vec2 position) {
  ...
  assert(areFloatsEqual(currentStatePos, this->position) &&
         "The position of the character is not consistent with the state");
  // Get the offset
...
void GameCharacter::MoveTo(glm::vec2 position, float velocity, float dt) {
  ...
  //	this->GetHealth().GetTotalHealthBar().SetCenter(this->GetHealth().GetTotalHealthBar().GetCenter()
  //+ offset);
...
void GameCharacter::MoveTo(glm::vec2 position, float velocity,
...
void GameCharacter::RotateTo(float angle, float angularVelocity, float dt) {
  ...
  assert((rest > 0.f && angularVelocity > 0.f) ||
         (rest < 0.f && angularVelocity < 0.f));
...
void GameCharacter::SetRelativeCenterRatio(GameCharacterState state, int id,
                                           glm::vec2 center) {
...
void GameCharacter::UpdateCarriedObjectsPosition() {
  ...
  for (auto& p : positions) {
    if (p.first == this->state) {
      assert(areFloatsEqual(this->position, p.second) &&
             "The position of the character is not consistent with the state");
    }
    characterCenters[p.first] = p.second + sizes[p.first] * 0.5f;
  }
  for (auto& object : carriedObjects) {
    assert(relativeCenterRatios.count(object.first) > 0 &&
           "The relative center ratio for the carried object is not set");
    for (auto& [state, relativeCenterRatio] :
         relativeCenterRatios[object.first]) {
      glm::vec2 objectCenter =
          characterCenters[state] + relativeCenterRatio * sizes[state];
...
void GameCharacter::UpdateHealthBarPosition() {
  if (this->health != nullptr) {
    ...
    assert(relativeCenterRatios.count(id) > 0 &&
           "The relative center ratio for the total health bar is not set");
    assert(relativeCenterRatios[id].count(GameCharacterState::FIGHTING) > 0 &&
           "The relative center ratio for the total health bar in the fighting "
           "state is not set");
    glm::vec2 relativeCenterRatio =
...
Texture2D GameCharacter::GetGeneralTexture(
    GameCharacterState otherState) const {
