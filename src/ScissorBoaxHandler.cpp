...
void ScissorBoxHandler::SetScissorBox(GLint x, GLint y, GLsizei width,
  ...
  // GLint actualX = x * actual_window_size_padding_.width /
  // expected_window_size_padding_.width; GLint actualY = y *
  // actual_window_size_padding_.height / expected_window_size_padding_.height;
  // GLsizei actualWidth = width * actual_window_size_padding_.width /
  // expected_window_size_padding_.width; GLsizei actualHeight = height *
  // actual_window_size_padding_.height / expected_window_size_padding_.height;
  ////actualX += actual_window_size_padding_.padLeft;
  ////actualY += actual_window_size_padding_.padBottom;

  // glScissor(actualX + actual_window_size_padding_.padLeft, actualY +
  // actual_window_size_padding_.padBottom, actualWidth, actualHeight);
  /*std::cout << "actualX: " << actualX << " actualY: " << actualY << "
  actualWidth: " << actualWidth << " actualHeight: " << actualHeight <<
  std::endl; std::cout << "actualX + padLeft: " << actualX +
  actual_window_size_padding_.padLeft << " actualY+padBottom: " << actualY +
  actual_window_size_padding_.padBottom << std::endl;*/
...
void ScissorBoxHandler::SetIntersectedScissorBox(GLint x, GLint y,
  ...
  intersection.height =
      ...
      intersection.y;
  // std::cout << "input: " << x << " " << y << " " << width << " " << height <<
  // std::endl; std::cout << "scissor_box: " << scissor_box_.x << " " <<
  // scissor_box_.y << " " << scissor_box_.width << " " << scissor_box_.height
  // << std::endl; std::cout << "intersection: " << intersection.x << " " <<
  ...
  /*assert(intersection.width >= 0 && intersection.height >= 0 && "Invalid
   * intersection of scissor boxes!");*/
