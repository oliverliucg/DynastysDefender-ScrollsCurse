...
Scroll::Scroll(glm::vec2 center, glm::vec2 rollerSize, glm::vec2 silkSize,
  ...
  topRoller = std::make_unique<GameObject>(
      /*pos=*/center -
          glm::vec2(rollerSize.x / 2.0f, silkLen / 2.0f + rollerSize.y * 0.85),
      /*size=*/rollerSize, /*roll=*/0.f,
      /*rotationPivot=*/glm::vec2(0.5f, 0.5f),
      /*velocity=*/glm::vec2(0.0f, 0.0f),
      /*color=*/glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), /*sprite=*/rollerTexture);
  bottomRoller = std::make_unique<GameObject>(
      /*pos=*/center + glm::vec2(-rollerSize.x / 2.0f,
                                 silkLen / 2.0f - rollerSize.y * 0.155),
      /*size=*/rollerSize, /*roll=*/0.f,
      /*rotationPivot=*/glm::vec2(0.5f, 0.5f),
      /*velocity=*/glm::vec2(0.0f, 0.0f),
      /*color=*/glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), /*sprite=*/rollerTexture);

  glm::vec2 rotationPivotForTopRoller =
      (center - topRoller->GetPosition()) / topRoller->GetSize();
  glm::vec2 rotationPivotForBottomRoller =
      (center - bottomRoller->GetPosition()) / bottomRoller->GetSize();
...
glm::vec2 Scroll::GetCenter() const { return center; }

glm::vec2 Scroll::GetRollerSize() const { return rollerSize; }

float Scroll::GetSilkLen() const { return silkSize.y; }

float Scroll::GetSilkWidth() const { return silkSize.x; }

glm::vec4 Scroll::GetSilkBounds() const {
  return glm::vec4(center.x - silkSize.x / 2.0f, center.y - silkSize.y / 2.0f,
                   center.x + silkSize.x / 2.0f, center.y + silkSize.y / 2.0f);
}

std::unique_ptr<GameObject>& Scroll::GetTopRoller() { return topRoller; }

std::unique_ptr<GameObject>& Scroll::GetBottomRoller() { return bottomRoller; }

void Scroll::SetSilkLen(float silkLen) {
  this->silkSize.y = silkLen;
  topRoller->SetPosition(
      center -
      glm::vec2(rollerSize.x / 2.0f, silkLen / 2.0f + rollerSize.y * 0.85));
  bottomRoller->SetPosition(
      center +
      glm::vec2(-rollerSize.x / 2.0f, silkLen / 2.0f - rollerSize.y * 0.155));
  if (silkLen == 0.f) {
    std::cout << "center: " << center.x << ", " << center.y << std::endl;
    std::cout << "toproller position: " << topRoller->GetPosition().x << ", "
              << topRoller->GetPosition().y << std::endl;
    std::cout << "bottomroller position: " << bottomRoller->GetPosition().x
              << ", " << bottomRoller->GetPosition().y << std::endl;
  }
  glm::vec2 rotationPivotForTopRoller =
      (center - topRoller->GetPosition()) / topRoller->GetSize();
  glm::vec2 rotationPivotForBottomRoller =
...
void Scroll::AddSilkLen(float delta) {
  ...
  bottomRoller->SetPosition(bottomRoller->GetPosition() +
                            glm::vec2(0, delta * 0.5));
...
glm::vec2 Scroll::GetTargetCenter(std::string target) const {
  assert(this->targetCenters.count(target) > 0 &&
         "The target center is not set.");
...
void Scroll::SetAlpha(float alpha) {
  this->color.a = alpha;
  topRoller->SetColor(glm::vec4(topRoller->GetColor().r,
                                topRoller->GetColor().g,
                                topRoller->GetColor().b, alpha));
  bottomRoller->SetColor(glm::vec4(bottomRoller->GetColor().r,
                                   bottomRoller->GetColor().g,
                                   bottomRoller->GetColor().b, alpha));
}
...
float Scroll::GetVelocityForNarrowing() const { return velocityForNarrowing; }

float Scroll::GetVelocityForClosing() const { return velocityForClosing; }

float Scroll::GetVelocityForOpening() const { return velocityForOpening; }
...
void Scroll::Move(float dt) {
  glm::vec2 distanceVector =
      calculateTravelDistanceVector(velocity, acceleration, dt);
