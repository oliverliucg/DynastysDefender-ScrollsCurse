...
// Ray is a class that represents a ray in 2D space. It starts from the bubble
// shooter and goes to the direction that the shooter is pointing at. It would
// bounce off the walls and end at the first static bubble that it hits. As it
// bounces off the walls, it would generate a list of points that represent the
// path that it takes. The path is used to draw the ray.
class Ray {
 public:
  ...
  void UpdatePath(glm::vec4 boundaries,
 ...
 private:
  ...
  std::vector<glm::vec2> GetBubbleCollisionPoint(
