...
bool Button::IsTextOnCenter() const {
	return text_on_center_;
}

bool Button::IsPositionInside(glm::vec2 position) const {
	return position.x >= position_.x && position.x <= position_.x + size_.x && position.y >= position_.y && position.y <= position_.y + size_.y;
}

void Button::Draw(std::shared_ptr<TextRenderer> textRenderer, std::shared_ptr<ColorRenderer> colorRenderer) {
	colorRenderer->DrawColor(position_, size_, 0.0f, glm::vec2(0.5f, 0.5f), color_);
