...
Bubble::Bubble(glm::vec2 pos, float radius, glm::vec2 velocity, glm::vec4 color, Texture2D sprite) : GameObject(pos, glm::vec2(radius * 2.0f, radius * 2.0f), 0.0f, glm::vec2(0.5f, 0.5f), velocity, color, sprite), radius(radius), state(BubbleState::kNormal) {}
...
bool Bubble::Move(float deltaTime, glm::vec4 boundaries, std::unordered_map<int, std::unique_ptr<Bubble> >& statics) {
	...
	for (auto& staticBubble : statics) {
		...
		// If the distance between the centers of the two bubbles is less than the sum of their radius, then the bubble is penetrating the static bubble.
		...
		if (distance < radius + staticRadius) {
			...
			// Get normalized vector of the line that connects the centers of the two bubbles
			...
			// Move the bubble back to the point of penetration. The point of penetration is the point that is on the surface of the static bubble and is on the line that connects the centers of the two bubbles. That means the distance between the moving bubble and the center of the static bubble is equal to the radius of the static bubble.
			// First we calculate the angle between the opposite direction and the line that connects the centers of the two bubbles.
			float cosGamma = glm::dot(oppositeDirection, normalizedLine);
			bool isAcute = cosGamma > 0.0f;
			cosGamma = glm::abs(cosGamma);
			// Use law of cosines to calculate the distance between the centers of the moving bubble at the point of penetration and at the current point.
			std::pair<float, float> distanceToPenetrationPoints = lawOfCosinesB(absoluteDistance, staticBubble.second->GetRadius() + this->GetRadius(), cosGamma);

			// If gamma is acute, then choose the longer distance (the second). Otherwise, choose the shorter distance (the first). 
			float distanceToPenetrationPoint = isAcute ? distanceToPenetrationPoints.second : distanceToPenetrationPoints.first;
			...
			// Calculate the time to move from the penetration point to the current point.
