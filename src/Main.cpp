...
void switchToWindowedMode(GLFWwindow* window, int width, int height) {
  ...
  glfwSetWindowMonitor(window, NULL, windowPosX, windowPosY, width, height,
                       GLFW_DONT_CARE);
}
...
void reconfigureWindowSize(GLFWwindow* window, int width, int height) {
  ...
  // auto expectedWindowSizePadding =
  // ScissorBoxHandler::GetInstance().GetExpectedWindowSizePadding(); auto
  // actualWindowSizePadding =
  // ScissorBoxHandler::GetInstance().GetActualWindowSizePadding();
