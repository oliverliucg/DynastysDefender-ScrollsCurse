...
Capsule::Capsule(glm::vec2 center, glm::vec2 size, glm::vec4 color) : GameObject(center-size/2.0f, size, 0.0f, glm::vec2(0.5f, 0.5f), glm::vec2(0.0f, 0.0f), color), center(center), rectangleVisible(true), topSemiCircleVisible(true), bottomSemiCircleVisible(true){
	
}

Capsule::Capsule(const Capsule& other) : GameObject(other), center(other.center), rectangleVisible(other.rectangleVisible), topSemiCircleVisible(other.topSemiCircleVisible), bottomSemiCircleVisible(other.bottomSemiCircleVisible) {
}
...
glm::vec2 Capsule::GetCenter() const {
	return center;
}
...
bool Capsule::IsRectangleVisible() const {
	return rectangleVisible;
}

bool Capsule::IsTopSemiCircleVisible() const {
	return topSemiCircleVisible;
}
...
bool Capsule::IsPositionInside(glm::vec2 mousePosition) {
	// If there's rotation for the capsule, the mouse position should be rotated back to the original position.
	if (this->roll != 0.0f) {
	...
	// The mouse cursor should be in the rectangle or the semi-circles of the capsule.
	...
	bool isInTopSemiCircle = glm::distance(glm::vec2(mousePosition.x, mousePosition.y), topSemiCircleCenter) < semiCircleRadius;
	bool isInBottomSemiCircle = glm::distance(glm::vec2(mousePosition.x, mousePosition.y), bottomSemiCircleCenter) < semiCircleRadius;

	return isInRectangle && rectangleVisible || isInTopSemiCircle && topSemiCircleVisible || isInBottomSemiCircle && bottomSemiCircleVisible;
}

void Capsule::Draw(std::shared_ptr<ColorRenderer> colorRenderer, std::shared_ptr<CircleRenderer> circleRenderer) {
	...
	if (topSemiCircleVisible)
		circleRenderer->DrawCircle(
			...
			GetRadius(),
			glm::pi<float>()+this->roll,
			this->color);

	// Draw the bottom semicircle if it is visible
	if (bottomSemiCircleVisible)
		circleRenderer->DrawCircle(
			rotateVector(GetBottomSemiCircleCenter(), this->roll, this->GetCenter()),
			GetRadius(),
			0.f+this->roll,
			this->color);
}

void Capsule::DrawOutline(std::shared_ptr<ColorRenderer> colorRenderer, std::shared_ptr<CircleRenderer> circleRenderer) {
	// Draw the rectangle if it is visible
	if (rectangleVisible)
		colorRenderer->DrawEdge(
			GetRectangleTopLeft(),
			GetRectangleSize(),
			this->roll,
			this->rotationPivot,
