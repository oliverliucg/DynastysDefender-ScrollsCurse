...
bool areFloatsEqual(const glm::vec3& a, const glm::vec3& b, float epsilon) {
	return areFloatsEqual(a.r, b.r, epsilon) && areFloatsEqual(a.g, b.g, epsilon) && areFloatsEqual(a.b, b.b, epsilon);
}
...
int getSignOfCrossProduct(glm::vec2 a, glm::vec2 b) {
	...
	// return 1 if crossProduct is positive, -1 if crossProduct is negative, and 0 if crossProduct is 0
    if (crossProduct > 0) {
		return 1;
	}
...
bool isSameColor(glm::vec3 rgb1, glm::vec3 rgb2) {
	return areFloatsEqual(rgb1.r, rgb2.r) && areFloatsEqual(rgb1.g, rgb2.g) && areFloatsEqual(rgb1.b, rgb2.b);
}
...
bool isPointOnLine(glm::vec2 point, glm::vec3 lineParams) {
	return areFloatsEqual(lineParams.x * point.x + lineParams.y * point.y + lineParams.z, 0.0f);
}
...
std::pair<int, int> findWord(std::string text, int startIndex) {
    ...
    // The word is end with a space or the end of the string or the end of the line.
    auto it = std::find_if(startIt, text.end(), [](char c) { return c == ' ' || c == '\n'; });
