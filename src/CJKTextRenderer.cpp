...
//void CJKTextRenderer::LoadLanguage(std::string font, unsigned int fontSize,
//                               CharStyle charStyle, Language language) {
//  assert(language == Language::CHINESE_SIMPLIFIED ||
//		 language == Language::CHINESE_TRADITIONAL || language == Language::KOREAN || language == Language::JAPANESE && "The language is not supported.");
//  // Load Engish font for now.
//  switch (language) {
...
//}
//
//void CJKTextRenderer::LoadPreferredLanguage(unsigned int fontSize, CharStyle charStyle) {
...
std::pair<glm::vec3, bool> CJKTextRenderer::GetTextSize(
    std::string text, float scale, float lineWidth, float lineSpacingFactor,
