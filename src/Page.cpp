...
Page::Page(const std::string& name) : name_(name), position_(glm::vec2(0.f)), top_spacing_(0.f), bottom_spacing_(0.f), left_spacing_(0.f), max_height_(std::numeric_limits<float>::max()) {}
...
float Page::GetLimitedSectionHeight(const std::string& section_name) const {
	assert(sections_.find(section_name) != sections_.end() && "Section not found");
	return std::min(sections_.at(section_name)->GetHeight(), sections_.at(section_name)->GetMaxHeight());
}
...
float Page::GetHeight() const {
	...
	for (size_t i = 0; i < order_.size(); ++i) {
		...
		if (i + 1 < order_.size()) {
			if (inter_section_spacing_.find(order_[i]) == inter_section_spacing_.end() ||
				inter_section_spacing_.at(order_[i]).find(order_[i + 1]) == inter_section_spacing_.at(order_[i]).end()) {
				continue;
			}
...
std::vector<std::string> Page::GetOrder() const {
	return order_;
}
...
void Page::SetInterSectionSpacing(const std::string& section1, const std::string& section2, float spacing) {
	inter_section_spacing_[section1][section2] = spacing;
	inter_section_spacing_[section2][section1] = spacing;
}

float Page::GetInterSectionSpacing(const std::string& section1, const std::string& section2) const {
	return inter_section_spacing_.at(section1).at(section2);
}

void Page::SetTopSpacing(float spacing) {
	top_spacing_ = spacing;
}

float Page::GetTopSpacing() const {
	return top_spacing_;
}

void Page::SetBottomSpacing(float spacing) {
	bottom_spacing_ = spacing;
}

float Page::GetBottomSpacing() const {
	return bottom_spacing_;
}

void Page::SetLeftSpacing(float spacing) {
	left_spacing_ = spacing;
}

float Page::GetLeftSpacing() const {
	return left_spacing_;
}

void Page::SetMaxHeight(float height) {
	max_height_ = height;
}

float Page::GetMaxHeight() const {
	return max_height_;
}
...
void Page::UpdatePosition() {
	SetPosition(GetPosition());
}

glm::vec2 Page::GetPosition() const {
	return position_;
}
