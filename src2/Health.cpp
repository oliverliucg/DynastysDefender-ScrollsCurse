...
Health::Health() : totalHealth(0), currentHealth(0), damageTextInitialPosition(glm::vec2(0.f)), damageTextTargetPosition(glm::vec2(0.f)), damageTextFadedPosition(glm::vec2(0.f)), damagePopOutToRight(true), damageTextScale(1.f) {}

Health::Health(int totalHealth) : totalHealth(totalHealth), currentHealth(totalHealth), damageTextInitialPosition(glm::vec2(0.f)), damageTextTargetPosition(glm::vec2(0.f)), damageTextFadedPosition(glm::vec2(0.f)), damagePopOutToRight(true), damageTextScale(1.f) {}

Health::Health(const Health& other) : totalHealth(other.totalHealth), currentHealth(other.currentHealth), damageTextInitialPosition(other.damageTextInitialPosition), damageTextTargetPosition(other.damageTextTargetPosition), damageTextFadedPosition(glm::vec2(0.f)), damagePopOutToRight(true), damageTextScale(1.f) {}

Health::Health(Health&& other) noexcept : totalHealth(other.totalHealth), currentHealth(other.currentHealth), damageTextInitialPosition(other.damageTextInitialPosition), damageTextTargetPosition(other.damageTextTargetPosition), damageTextFadedPosition(glm::vec2(0.f)), damagePopOutToRight(true), damageTextScale(1.f) {}
...
void Health::SetTotalHealth(int totalHealth) { this->totalHealth = totalHealth; }
...
void Health::SetCurrentHealth(int currentHealth) {
	...
	assert(this->currentHealth <= totalHealth && this->currentHealth >= 0 && "the health should be between 0 and the total health");
	// Adjust the size of the current health bar based on the proportion of the current health to the total health
	...
	// Get the size of the current health bar based on the proportion of the current health to the total health
	currentHealthBar.SetSize(glm::vec2(totalHealthBar.GetSize().x, totalHealthBar.GetSize().y * proportion));

	// The position is always the same as the position of the total health bar
	currentHealthBar.SetCenter(totalHealthBar.GetCenter() - glm::vec2(totalHealthBar.GetSize().y * (1.f-proportion) / 2.0f, 0));
...
void Health::SetTotalHealthBar(glm::vec2 pos, glm::vec2 size, glm::vec4 color) {
	...
	// Set the initial position and target position and the faded position of the damage text
...
void Health::SetCurrentHealthBar(glm::vec2 pos, glm::vec2 size, glm::vec4 color) {
...
void Health::IncreaseHealth(int mount) {
	...
	damageTexts.push_back(Text(/*pos=*/this->totalHealthBar.GetCenter() + glm::vec2(0, 5 * kBubbleRadius),
		/*lineWidth=*/10 * kBubbleRadius));
	auto& damageText = damageTexts.back();
	damageText.SetPosition(this->totalHealthBar.GetCenter() + glm::vec2(0, 5 * kBubbleRadius));
...
void Health::DecreaseHealth(int mount) {
	this->IncreaseHealth(-mount);
}

void Health::SetDamagePopOutDirection(bool popOutToRight) {
	damagePopOutToRight = popOutToRight;
	if (!damagePopOutToRight) {
		damageTextInitialPosition = totalHealthBar.GetCenter() + glm::vec2(-kBubbleRadius, 5.5f * kBubbleRadius);
		damageTextTargetPosition = rotateVector(totalHealthBar.GetBottomSemiCircleCenter(), totalHealthBar.GetRoll(), totalHealthBar.GetCenter()) + glm::vec2(-kBubbleRadius, 2.7f * kBubbleRadius);
	}
	else {
		damageTextInitialPosition = totalHealthBar.GetCenter() + glm::vec2(0, 5.5f * kBubbleRadius);
		damageTextTargetPosition = rotateVector(totalHealthBar.GetTopSemiCircleCenter(), totalHealthBar.GetRoll(), totalHealthBar.GetCenter()) + glm::vec2(0.f, 2.7f * kBubbleRadius);
	}
	damageTextFadedPosition = damageTextTargetPosition - glm::vec2(0.0f, 5 * kBubbleRadius);
}
...
void Health::UpdateDamageTexts(float dt) {
	...
	for (size_t i = 0; i < damageTexts.size(); ++i) {
		...
		assert(damageText.GetAlpha() > 0.f && "the alpha of the damage text should be greater than 0.");
		/*existingDamageTexts.emplace_back(damageText);*/
		bool isFadingOut = damageText.GetPosition().x == damageTextTargetPosition.x;
		glm::vec2 targetPosition = isFadingOut ? damageTextFadedPosition : damageTextTargetPosition;
		glm::vec2 initialPosition = isFadingOut ? damageTextTargetPosition : damageTextInitialPosition;
		...
		glm::vec2 currentDirection = glm::normalize(targetPosition - damageText.GetPosition());
		...
		if (innerProduct > 0) {
			// If the inner product is greater than 0, the damage text is still moving the target position
			damageText.SetPosition(newPosition);
			if (!isFadingOut) {
				// Increase the scale of the damage text based on the proportion of the distance from the target position to the initial position to the distance from the current position to the initial position
				float scale = damageTextScale * newDistance / totalDistance;
				damageText.SetScale(scale);
			}
			else {
				// Decrease the alpha of the damage text based on the proportion of the distance from the target position to the initial position to the distance from the current position to the initial position
			...
			if (!isFadingOut) {
				// If the inner product is less than 0, set the position of the damage text to the target position
...
void Health::DrawHealthBar(std::shared_ptr<ColorRenderer> colorRenderer, std::shared_ptr<CircleRenderer> circleRenderer) {
