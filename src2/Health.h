...
public:
	...
	Health();
	// Constructs a new Health instance. Requires the total health of the character.
	...
	// Move the newly Created damage text towards the bottom right of the health bar, and then move it upwards and fade it out.
	void UpdateDamageTexts(float dt);
	// Draw the health bar. The total health is represented by an edge-only capsule, and the current health is represented by a filled capsule. 
	void DrawHealthBar(std::shared_ptr<ColorRenderer> colorRenderer, std::shared_ptr<CircleRenderer> circleRenderer);
