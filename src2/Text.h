...
public:
	...
	Text(glm::vec2 pos, float lineWidth, glm::vec4 boxBounds = glm::vec4(0.f,0.f,kWindowSize), glm::vec3 color = glm::vec3(0.f), float scale = 1.0f, float lineSpacingFactor = 1.2f, float additionalPadding = kBubbleRadius / 3.0f);
	// Copy constructor
	...
	void Draw(std::shared_ptr<TextRenderer> textRenderer, bool textCenteringEnabled = false);
...
private:
	...
	// The linear relationship between the center of the scroll icon and the position of the text.
