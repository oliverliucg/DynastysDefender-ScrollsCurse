...
// GameObject contains all state information relevant to a game object as shown by its variable names.
class GameObject {
public:
	// Constructor(s)
	GameObject();
	GameObject(glm::vec2 pos, glm::vec2 size, float roll = 0.0f, glm::vec2 rotationPivot = glm::vec2(0.5f, 0.5f), glm::vec2 velocity = glm::vec2(0.0f, 0.0f), glm::vec4 color = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), Texture2D sprite = Texture2D());
	// Copy constructor
	...
	virtual void Draw(std::shared_ptr<SpriteDynamicRenderer> renderer, glm::vec4 texCoords);
