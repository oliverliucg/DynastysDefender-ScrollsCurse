...
Shooter::Shooter(glm::vec2 pos, glm::vec2 size, glm::vec2 rotationPivot, glm::vec2 tailPivot, Texture2D sprite) : GameObject(pos, size, 0.0f, rotationPivot, glm::vec2(0.0f, 0.0f), glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), sprite)
{
	glm::vec2 rotationCenter(position + rotationPivot * size);
	glm::vec2 tail(position + tailPivot * size);
	carriedBubble = Bubble(glm::vec2(rotationCenter.x - kBubbleRadius, rotationCenter.y - kBubbleRadius), kBubbleRadius, glm::vec2(0.0f, 0.0f), glm::vec4(0.0f, 1.0f, 0.0f, 0.8f), ResourceManager::GetInstance().GetTexture("bubble"));
	nextBubble = Bubble(glm::vec2(tail.x - kBubbleRadius/2, tail.y - kBubbleRadius/2), kBubbleRadius/2, glm::vec2(0.0f, 0.0f), glm::vec4(0.0f, 0.0f, 1.0f, 0.7f), ResourceManager::GetInstance().GetTexture("bubble"));
	// Calculate the rotation pivot for the next bubble. This rotation pivot is the point that the next bubble rotates around.
	...
	// Initialize the ray
	ray = Ray(rotationCenter, GetShootingDirection(), glm::vec4(colorMap[Color::White], 1.0f));
}
...
std::unique_ptr<Bubble> Shooter::ShootBubble() {
	std::unique_ptr<Bubble> copiedBubble = std::make_unique<Bubble>(carriedBubble);
	copiedBubble->SetVelocity(this->GetShootingDirection() * 16.0f * kVelocityUnit);
	// RGB
	glm::vec3 rgb(nextBubble.GetColor().r, nextBubble.GetColor().g, nextBubble.GetColor().b);
...
void Shooter::GenerateBubble() {
	...
	//// Otherwise, generate a new bubble. The bubble is always at the rotation center of the shooter. The radius of it is equal to the regular bubble radius provided by the ResourceManager.
	//glm::vec2 rotationCenter(position.x+rotationPivot.x*size.x, position.y+rotationPivot.y*size.y);
	//carriedBubble = std::make_unique<Bubble>(glm::vec2(rotationCenter.x - kBubbleRadius, rotationCenter.y - kBubbleRadius), kBubbleRadius, glm::vec2(0.0f, 0.0f), glm::vec4(0.0f, 1.0f, 0.0f, 0.8f), ResourceManager::GetInstance().GetTexture("bubble"));
}
...
void Shooter::SetPosition(glm::vec2 position) {
	...
	carriedBubble.SetPosition(glm::vec2(rotationCenter.x - kBubbleRadius, rotationCenter.y - kBubbleRadius));
	// Set the position for the next bubble based on its offset from the carried bubble
...
glm::vec4 Shooter::GetNewBubbleColor(const std::unordered_map<int, std::unique_ptr<Bubble>>& statics) {
	// Create a color map for each color's probability
	std::unordered_map<Color, float> colorProbalibity;
	// Iterate thourgh the color map and initialize the probability for each color to 0
	float total = 0.0f;
	...
	// Iterate through the color map and find the color that the random float falls into
	float sum = 0.0f;
...
const Bubble& Shooter::GetCarriedBubble() const {
	return carriedBubble;
}
