...
CircleRenderer::CircleRenderer(const Shader& shader, float percentageOfCircle) : numSegments(static_cast<size_t>(180.0f * percentageOfCircle)), Renderer(shader)
{
...
void CircleRenderer::DrawCircle(glm::vec2 position, float radius, float rotate, glm::vec4 color)
{
	...
	glDrawArrays(GL_TRIANGLE_FAN, 0, numSegments + 2); // +2 for center and closing 
	glBindVertexArray(0);
}

void CircleRenderer::DrawCircleEdge(glm::vec2 position, float radius, float rotate, glm::vec4 color) {
	// Prepare transformations
	this->shader.Use();
	glm::mat4 model = glm::mat4(1.0f);
	model = glm::translate(model, glm::vec3(position, 0.0f));
